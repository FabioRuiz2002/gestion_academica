    ...
    <div class="card mb-3 bg-light border-0">
        <div class="card-body p-2">
            <div class="input-group">
                ...
                <input type="text" id="inputBusqueda" class="form-control border-start-0" placeholder="Buscar por nombre, DNI o código..." onkeyup="filtrarUsuarios()">
            </div>
        </div>
    </div>
                                ...
                                <?php foreach ($escuelas as $escuela => $estudiantes): ?>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-sm table-hover align-middle">
                                            <thead class="table-light"><tr><th>Código</th><th>DNI</th><th>Nombre</th><th>Email</th><th class="text-end">Acciones</th></tr></thead>
                                            <tbody>
                                                <?php foreach ($estudiantes as $row): 
                                                    $textoBusqueda = strtolower($row['dni'] . ' ' . $row['nombre'] . ' ' . $row['apellido'] . ' ' . $row['codigo_estudiante']);
                                                ?>
                                                    <tr class="usuario-item" data-texto="<?php echo $textoBusqueda; ?>">
                                                        ...
                                                        <td class="text-end">
                                                            <a href="index.php?controller=Admin&action=gestionarMatriculaAlumno&id_estudiante=<?php echo $row['id_usuario']; ?>" class="btn btn-info btn-sm text-white" title="Ver Matrícula">
        ...
        <div class="tab-pane fade" id="profesores">
            <div class="table-responsive">
                <table class="table table-striped align-middle">
                    <thead class="table-dark"><tr><th>DNI</th><th>Nombre</th><th>Email</th><th class="text-end">Acciones</th></tr></thead>
                    <tbody>
                        <?php foreach ($listaProfesores as $row): 
                             $textoBusqueda = strtolower($row['dni'] . ' ' . $row['nombre'] . ' ' . $row['apellido']);
                        ?>
                            <tr class="usuario-item" data-texto="<?php echo $textoBusqueda; ?>">
                                <td><?php echo htmlspecialchars($row['dni']); ?></td>
                                <td><?php echo htmlspecialchars($row['apellido'] . ', ' . $row['nombre']); ?></td>
...
function filtrarUsuarios() {
    ...
    let filas = document.querySelectorAll('.usuario-item');
    
    filas.forEach(fila => {
        let texto = fila.getAttribute('data-texto');
    ...
    // Abrir acordeones si hay búsqueda
    let acordeones = document.querySelectorAll('.accordion-collapse');
    acordeones.forEach(acc => {
        if (input.length > 0) acc.classList.add('show');
        else acc.classList.remove('show');
    });
}
