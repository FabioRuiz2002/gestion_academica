    ...
    <div class="tab-content mt-3" id="userTabsContent">
            ...
            <div class="accordion" id="accordionFacultades">
                ...
                <?php foreach ($listaEstudiantesAgrupados as $facultad => $escuelas): $fid = md5($facultad); ?>
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="h-<?php echo $fid; ?>">
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#c-<?php echo $fid; ?>">
                            ...
                            <div class="accordion-body">
                                <?php foreach ($escuelas as $escuela => $estudiantes): ?>
                                    ...
                                    <table class="table table-sm table-hover">
                                        <thead class="table-light"><tr><th>ID</th><th>Código</th><th>DNI</th><th>Nombre</th><th>Email</th><th>Acción</th></tr></thead>
                                        <tbody>
        ...
        <div class="tab-pane fade" id="profesores">
            <table class="table table-striped">
                <thead class="table-dark"><tr><th>ID</th><th>DNI</th><th>Nombre</th><th>Email</th><th>Acción</th></tr></thead>
                <tbody>
                    <?php foreach ($listaProfesores as $row): ?>
                        <tr>
                            <td><?php echo $row['id_usuario']; ?></td>
                            <td><?php echo htmlspecialchars($row['dni']); ?></td>
                            <td><?php echo htmlspecialchars($row['apellido'] . ', ' . $row['nombre']); ?></td>
        ...
        <div class="tab-pane fade" id="admins">
            <table class="table table-striped">
                <thead class="table-dark"><tr><th>ID</th><th>DNI</th><th>Nombre</th><th>Email</th><th>Acción</th></tr></thead>
                <tbody>
                    <?php foreach ($listaAdmins as $row): ?>
                        <tr>
                            <td><?php echo $row['id_usuario']; ?></td>
                            <td><?php echo htmlspecialchars($row['dni']); ?></td>
                            <td><?php echo htmlspecialchars($row['apellido'] . ', ' . $row['nombre']); ?></td>
        ...
        <div class="modal-content">
            <form action="index.php?controller=Admin&action=crearUsuario" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3"><label>Password</label><input type="password" name="password" class="form-control" required pattern="(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Min 8 car, 1 May, 1 min"></div>
                    <div class="mb-3">
                        <label>Rol</label>
                        <select name="id_rol" id="rol_crear" class="form-select" required onchange="togglePlan('crear')">
                            <option value="">Seleccione...</option>
                            <?php foreach($listaRoles as $r): ?><option value="<?php echo $r['id_rol']; ?>"><?php echo $r['nombre_rol']; ?></option><?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-3" id="plan_crear" style="display:none;">
                        <label>Plan de Estudio</label>
                        <select name="id_plan_estudio" class="form-select">
                            <option value="">Seleccione...</option>
                            <?php foreach($listaPlanes as $p): ?><option value="<?php echo $p['id_plan_estudio']; ?>"><?php echo htmlspecialchars($p['nombre_escuela'].' - '.$p['nombre_plan']); ?></option><?php endforeach; ?>
                        </select>
                    </div>
                </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="index.php?controller=Admin&action=editarUsuario" method="POST">
                ...
                <div class="modal-body">
                    <input type="hidden" name="edit_id_usuario" id="edit_id">
                    <div class="mb-3"><label>Nombre</label><input type="text" name="edit_nombre" id="edit_nombre" class="form-control" required></div>
                    <div class="mb-3"><label>Apellido</label><input type="text" name="edit_apellido" id="edit_apellido" class="form-control" required></div>
                    <div class="mb-3"><label>DNI</label><input type="text" name="edit_dni" id="edit_dni" class="form-control" required></div>
                    <div class="mb-3"><label>Email</label><input type="email" name="edit_email" id="edit_email" class="form-control" required></div>
                    <div class="mb-3"><label>Password (Dejar vacío para no cambiar)</label><input type="password" name="edit_password" class="form-control"></div>
                    <div class="mb-3">
                        <label>Rol</label>
                        <select name="edit_id_rol" id="edit_rol" class="form-select" required onchange="togglePlan('editar')">
                            <?php foreach($listaRoles as $r): ?><option value="<?php echo $r['id_rol']; ?>"><?php echo $r['nombre_rol']; ?></option><?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-3" id="plan_editar" style="display:none;">
                        <label>Plan de Estudio</label>
                        <select name="edit_id_plan_estudio" id="edit_plan" class="form-select">
                            <option value="">Seleccione...</option>
                            <?php foreach($listaPlanes as $p): ?><option value="<?php echo $p['id_plan_estudio']; ?>"><?php echo htmlspecialchars($p['nombre_escuela'].' - '.$p['nombre_plan']); ?></option><?php endforeach; ?>
                        </select>
                    </div>
...
<script>
function togglePlan(modo) {
    let rol = document.getElementById(modo === 'crear' ? 'rol_crear' : 'edit_rol').value;
    let div = document.getElementById(modo === 'crear' ? 'plan_crear' : 'plan_editar');
    div.style.display = (rol == 3) ? 'block' : 'none';
}
function cargarUsuario(id) {
    fetch('index.php?controller=Admin&action=getUsuario&id='+id).then(r=>r.json()).then(d=>{
        ...
        document.getElementById('edit_dni').value=d.dni;
        document.getElementById('edit_email').value=d.email;
        document.getElementById('edit_rol').value=d.id_rol;
        document.getElementById('edit_plan').value=d.id_plan_estudio;
        togglePlan('editar');
    });
}
function eliminarUsuario(id) {
    if(confirm('¿Eliminar?')) {
        fetch('index.php?controller=Admin&action=eliminarUsuario', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'id_usuario='+id})
